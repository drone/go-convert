...
func TestConvertStepRestoreCache(t *testing.T) {
	tests := []struct {
		name     string
		step     *v0.Step
		...
		{
			name: "with optional archive format and fail if key not found",
			step: &v0.Step{
				Spec: &v0.StepRestoreCache{
					Key:               "myApp-{{ checksum \"pom.xml\" }}",
					ArchiveFormat:     "Zstd",
					FailIfKeyNotFound: &flexible.Field[bool]{Value: true},
				},
			},
			expected: map[string]interface{}{
				"cachekey":              "myApp-{{ checksum \"pom.xml\" }}",
				"archiveformat":         "zstd",
				"fail_if_key_not_found": &flexible.Field[bool]{Value: true},
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ConvertStepRestoreCache(tt.step)
			if result == nil {
				t.Fatal("expected non-nil result")
			}

			if result.Uses != "restoreCache" {
				t.Errorf("expected Uses to be restoreCache, got %s", result.Uses)
			}
...
func TestConvertStepRestoreCache_Container(t *testing.T) {
	step := &v0.Step{
		Spec: &v0.StepRestoreCache{
			Key:       "cache-key",
	...
	result := ConvertStepRestoreCache(step)
	if result == nil {
		t.Fatal("expected non-nil result")
	}
...
func TestConvertStepRestoreCache_NilCases(t *testing.T) {
	tests := []struct {
		name string
		step *v0.Step
	}{
