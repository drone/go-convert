...
func TestParseFile(t *testing.T) {
	got, err := ParseFile("testdata/hello.yaml")
	if err != nil {
		t.Error(err)
	}

	want := &Pipeline{
		Stages: []string{"greeting"},
		Jobs: map[string]*Job{
			"en": {
				Script: Stringorslice{"echo hello world"},
				Stage:  "greeting",
			},
			"fr": {
				Script: Stringorslice{"echo bonjour monde"},
				Stage:  "greeting",
			},
		},
	}
...
func TestParseString(t *testing.T) {
	out, _ := ioutil.ReadFile("testdata/hello.yaml")
